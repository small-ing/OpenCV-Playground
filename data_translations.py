...
def collect_data(batch_size=10, offset=0):
    empty_marks = np.zeros((26, 21, batch_size, 2))
                    ...
                    for joint in range(26): # splits original joints
                        for dim in range(2): # splits x/y
                            if landmark_map[joint] != None: # checks if joint is in new landmark map
                                if empty_marks["abcdefghijklmnopqrstuvwxyz".index(letter)][landmark_map[joint]][index-offset][dim] == 0:
                                    empty_marks["abcdefghijklmnopqrstuvwxyz".index(letter)][landmark_map[joint]][index-offset][dim] = data["Landmarks"][joint][dim]  
